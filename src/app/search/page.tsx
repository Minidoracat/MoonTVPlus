...
function SearchPageClient() {
  ...
  const getAliasParam = () => {
    // 只搜私人影库时 alias 只会打公开豆瓣/TMDB，不会用于 Emby/OpenList
    if (privateLibraryOnly) return '';
  ...
  useEffect(() => {
    ...
    if (query) {
      ...
      if (currentFluidSearch) {
        // 流式搜索：打开新的流式连接
        const searchUrl = `/api/search/ws?q=${encodeURIComponent(trimmed)}${aliasParam}${
        ...
        // 传统搜索：使用普通接口
        const searchUrl = `/api/search?q=${encodeURIComponent(trimmed)}${aliasParam}${
          privateLibraryOnly ? '&privateOnly=1' : ''
        }`;
